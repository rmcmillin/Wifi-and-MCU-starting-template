...
void nm_spi_init(void)
{
    ...
    {
		log_error("Invalid chip rev, expected %lx, read %lx\n", SUPPORTED_WINC1500_CHIP_REVISION, chipId);
...
//		printInteger(*b);
	...
	//}
	//printString("\r\n");
	//log_info("spi:read %d - %d",sz, *b);
}

static void nmi_spi_write(uint8_t* b, uint16_t sz)
{	
	//log_info("spi:write %d",sz);
	//printString("Tx[");
	//printInteger(sz);
	//printString("]: ");
	//for (uint8_t i=0; i<sz; i++){
...
static int8_t spi_data_rsp(void)
{
    ...
    {
		log_error("[nmi spi]: Failed data response read, %x %x %x\n",rsp[0],rsp[1],rsp[2]);
    ...
    **/
    ...
    do {		
        ...
        **/
        ...
        if (retry <= 0) {	
			log_error("[nmi spi]: Failed data response read...(%h) retry:%d\r\n", rsp, retry)	;
...
static int8_t spi_data_write(uint8_t *b, uint16_t sz)
{
	...
	//log_trace("spi:write %d bytes",sz);
    //uint8_t rsp;
...
_RETRY_:
    ...
    **/
    ...
    result = spi_cmd(cmd, addr, 0, size,0);
    if (result != N_OK) {
		log_error("[nmi spi]: Failed cmd, write block (%08x)...\r\n", (unsigned int)addr);
    ...
    result = spi_cmd_rsp(cmd);
    if (result != N_OK) {
		log_error("[nmi spi ]: Failed cmd response, write block (%08x)...\r\n", (unsigned int)addr);
    ...
    **/
    ...
    if (result != N_OK) {
		log_error("[nmi spi]: Failed block data write...\r\n");
