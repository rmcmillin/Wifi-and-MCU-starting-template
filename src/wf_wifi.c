...
static bool InitStateMachine(void)
{
    ...
    switch (GetInitState())
    {
        ...
        case INIT_WAIT_FOR_CHIP_RESET_STATE:
			log_trace("INIT_WAIT_FOR_CHIP_RESET_STATE");
        ...
        case INIT_WAIT_FOR_BOOT_ROM_STATE:
			log_trace("INIT_WAIT_FOR_BOOT_ROM_STATE");
		...
		log_trace("INIT_WAIT_FOR_FIRMWARE_START_STATE");
            if (FirmwareStartStateMachine() == true)
            {
				...
				log_info("WINC1500 Host Driver:");
				log_info("  Chip ID:                     %3lx", rev.u32Chipid);
				log_info("  Firmware Version:            %d.%d.%d", rev.u8FirmwareMajor, rev.u8FirmwareMinor, rev.u8FirmwarePatch);
				log_info("  Firmware Build Date/Time:    %s, %s", rev.BuildDate, rev.BuildTime);
				log_info("  Firmware Min Driver Version: %d.%d.%d", rev.u8DriverMajor, rev.u8DriverMinor, rev.u8DriverPatch);
				log_info("  Host Driver Version:         %d.%d.%d", M2M_FIRMWARE_VERSION_MAJOR_NO, M2M_FIRMWARE_VERSION_MINOR_NO, M2M_FIRMWARE_VERSION_PATCH_NO);
				log_info("  Host Driver Build Date/Time: %s, %s", __DATE__, __TIME__);                												
...
static bool ChipHardwareResetStateMachine(void)
{
    ...
    switch (GetChipHardwareResetState())
    {
        ...
        case CHIP_HARDWARE_RESET_FIRST_DELAY_1_MS:
            if (m2m_get_elapsed_time(startTime) >= 2) //2
            {
        ...
        case CHIP_HARDWARE_RESET_SECOND_DELAY_5_MS:
            if (m2m_get_elapsed_time(startTime) >= 6)   //6         
            {
        ...
        case CHIP_HARDWARE_RESET_FINAL_DELAY:
            if (m2m_get_elapsed_time(startTime) >= 10) //10
            {                        
...
static bool BootRomStateMachine(void)
{
    ...
    switch (GetBootRomState())
    {
        case BOOT_ROM_START:
			...
			log_trace("BOOT_ROM_START: %d", reg);			
            if (reg & 0x80000000)
            {
		...
		log_info("BOOT_ROM_WAIT_LOAD");
            ...
            {
				log_error("failed to load firmware from flash");
...
static bool FirmwareStartStateMachine(void)
{
    ...
    switch (GetFirmwareStartState())
    {
        ...
        case FIRMWARE_START_WAIT:
            ...
            {
				log_error("Time out for waiting for WINC1500 firmware to run");
...
void WifiInternalEventHandler(uint8_t opCode, uint16_t dataSize, uint32_t address)
{
    ...
    switch (opCode)
    {
        ...
        default:
		log_error("ERROR: invalid wifi op code");
...
int8_t nm_drv_deinit(void)
{
    ...
    if (M2M_SUCCESS != ret) 
    {
		log_warn("[nmi stop]: SPI flash disable fail");
...
void m2m_wifi_req_hidden_ssid_scan(uint8_t ch, uint8_t *p_ssidList)
{
    ...
    if(apNum <= M2M_WIFI_MAX_HIDDEN_SITES)
    {
        ...
        while(apNum)
        {
            ...
            { 
				log_warn("request_scan_ssid_list: failed");
...
static bool isValidApParameters(const tstrM2MAPConfig* apConfig)
{
    ...
    if(!(apConfig->au8DHCPServerIP[0] || apConfig->au8DHCPServerIP[1]))
    {
        ...
        {
			log_warn("Invalid dhcp server ip");
    ...
    if(apConfig->u8SecType != M2M_WIFI_SEC_OPEN)
    {
        if (apConfig->u8SecType == M2M_WIFI_SEC_WEP)
        {
            ...
            {
				log_warn("invalid key string size");
...
static int8_t ValidateScanOptions(tstrM2MScanOption* ptstrM2MScanOption)
{
    ...
    {
		log_warn("invalid number of scan slots");
    ...
    {
		log_warn("Invalid scan slot time");
    ...
    {
		log_warn("Invalid no of probe request per scan slot");
    ...
    {
		log_warn("Invalid rssi threshold");
...
static bool isConnectionParamsValid(const char *pcSsid, uint8_t u8SsidLen, uint8_t u8SecType, void *pvAuthInfo, uint16_t channel)
{
    ...
    {
		log_warn("Invalid security type");
    ...
    {
		log_warn("pv auth info cannot be null if using open security");
    ...
    if(u8SecType == M2M_WIFI_SEC_WEP)
    {
        ...
        {
			log_warn("Invalid wep key index");
        ...
        {
			log_warn("Invalid wep key length");
    ...
    else if(u8SecType == M2M_WIFI_SEC_WPA_PSK)
    {
        ...
        {
			log_warn("incorrect psk key length");
